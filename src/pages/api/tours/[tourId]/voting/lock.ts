...
/**
 * POST /api/tours/{tourId}/voting/lock
...
export const POST: APIRoute = async ({ params, locals, request }) => {
  ...
  if (!user) {
    return new Response(
      JSON.stringify({
        error: {
          code: "UNAUTHORIZED",
          message: "You must be logged in to lock voting.",
        },
      }),
  ...
  // Lock voting
  const { data, error } = await tourService.lockVoting(supabase, tourId);

  if (error || !data) {
    return new Response(
      JSON.stringify({
        error: {
          code: "LOCK_VOTING_FAILED",
          message: error?.message || "Failed to lock voting.",
        },
      }),
