...
5.  The backend checks if a user with that email exists.
    ...
    - **If user does not exist (Registration):** The backend asks Supabase to send a confirmation magic link. A new user record is created in `auth.users`, and a corresponding profile is created in the `profiles` table.
...
8.  **For new users:** Supabase authenticates the user and redirects them to the `/welcome` onboarding page.
...
- **`POST /api/auth/magic-link` (New)**
  ...
  - **Logic:**
    ...
    3.  If no, call `supabase.auth.signUp`, configuring the redirect to `/welcome`.
  - **Response:**
    - `200 OK`: On success.
...
- **Logic:**
  ...
  5.  This makes user data available in all Astro pages and API endpoints.
  6.  It will also handle protected routes. If a user tries to access a protected page without being authenticated, they will be redirected to `/login`.
  7.  It will refresh the token if necessary.
...
- **Table: `profiles`**
  ...
  - **Trigger:** A database trigger will be created. On a new user insertion into `auth.users`, a new row will be created in the `profiles` table. The `id` will be from `NEW.id`.
...
- The Site URL and Redirect URLs in the Supabase dashboard will be configured to point to the deployed application's domain.
